...
function initDb() {
    ...
    if (isPostgres) {
        db.query(studentTable)
            ...
            // Migration: Chuyển dữ liệu từ cột cũ sang cột mới nếu cần
            .then(() => db.query("UPDATE submissions SET file_paths = file_path WHERE file_paths IS NULL AND file_path IS NOT NULL").catch(() => {}))
            .then(() => seedStudents())
        ...
        db.serialize(() => {
            ...
            db.run("ALTER TABLE submissions ADD COLUMN file_paths TEXT", (err) => {
                // Sau khi thêm cột (hoặc nếu đã có), cập nhật dữ liệu cũ
                db.run("UPDATE submissions SET file_paths = file_path WHERE file_paths IS NULL AND file_path IS NOT NULL", (err) => {});
            });
            seedStudents();
        });
    }
